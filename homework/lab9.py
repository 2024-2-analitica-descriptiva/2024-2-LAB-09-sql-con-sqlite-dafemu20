...
def lab9():
    ...
    pregunta1 = cur.execute("SELECT sum(c12) AS 'SUM(c12)' FROM tbl1").fetchall()

    pregunta2 = cur.execute(
        """ 
    SELECT count(*) AS 'COUNT(*)' FROM tbl1;
    """
    ).fetchall()

    pregunta3 = cur.execute("SELECT * FROM tbl1 ORDER BY c14 LIMIT 5;").fetchall()

    pregunta4 = cur.execute(
        """ 
    SELECT K0, c16 FROM tbl1 WHERE K0 = SUBSTR(c16,1,1)
    """
    ).fetchall()

    pregunta5 = cur.execute(
        "SELECT * FROM tbl0 WHERE c02 = 100 OR c02 = 600"
    ).fetchall()

    pregunta6 = cur.execute("SELECT * FROM tbl1 WHERE K0 = 'A' ORDER BY c14").fetchall()

    pregunta7 = cur.execute(
        """ 
    SELECT * FROM tbl1 WHERE K0 != 'A' AND K0 != 'B' AND c13 != 200 AND c13 != 900 ORDER BY c14
    """
    ).fetchall()

    pregunta8 = cur.execute(
        """ 
    SELECT
        strftime('%Y', c23),
        avg(c21)
    ...
    pregunta9 = cur.execute(
        "SELECT * FROM tbl2 WHERE c21 = (SELECT min(c21) FROM tbl2)"
    ).fetchall()

    pregunta10 = cur.execute("SELECT * FROM tbl0 WHERE c02 >= 300").fetchall()

    pregunta11 = cur.execute(
        """
    SELECT
        strftime ('%Y', c14),
        COUNT(*) as 'CANT'
    ...
    pregunta12 = cur.execute(
        "SELECT K0, max(c12) AS 'MAX(c12)',min(c12) AS 'MIN(c12)' FROM tbl1 GROUP BY K0"
    ).fetchall()

    pregunta13 = cur.execute(
        "SELECT K0, ROUND(AVG(c12) + 0.0001, 2) AS "avg(c12)" FROM tbl1  WHERE c13>400 GROUP BY K0 ORDER BY K0;"
    ).fetchall()

    pregunta14 = cur.execute(
        """
    SELECT 
        tbl1.K0,
